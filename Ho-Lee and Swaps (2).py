...
#Function to fit the line of best fit on the parameters
def treas_swap_line (t_swaps,b1,b2,b3,b4):
    return(b1 + b2*t_swaps)*np.exp(-b3*t_swaps) + b4
...
#so (1- discount factor for n years)/sum of discount facotrs
# Function to calculate swap price
def swap_price(b_yield_arr, n):
    discArr = np.empty([0,0])
    count = 0
    for i in range(n - 1):
        count += 1
        b_yield_arr[i] = line(zz[i], popt[0], popt[1], popt[2], popt[3])
        discount = 1 / (1 + b_yield_arr[i] ** count)
        discArr = np.append(discArr, discount)

    end = 1 / (1 + b_yield_arr[n - 1] ** n)
    swap_px = (1 - end) / (np.sum(discArr))
    print(swap_px)
    return swap_px
...
for i in range(length_zz):
    n = i + 1
    print('\nThe swap price for year', i + 1, 'is: ', swap_r[i])
    print('The price for year ', i + 1, 'is: ', 100 - swap_price(b_yield_arr, n))
    print('The swap price for year', i + 1, 'is: ', swap_price(b_yield_arr,n))
    z = swap_price(b_yield_arr, i + 1) - swap_r[i]
    if (z != 0):
