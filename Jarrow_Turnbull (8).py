...
def CDS_spread(premium, prot):   
    spread = np.zeros(len(prot))
    for i in range (len(prot)):
            spread[i] = ((1-recovery)*prot[i])/premium[i]
    return spread
s = CDS_spread(premium, prot)
jarrow1['Model spread'] = s
jarrow1
...
#Convert MID to Market Spread
jarrow1['Mkt spread'] = jarrow1['MID']/1000
jarrow1
...
#To complete the program, we solve for lambda (default intensity) such that Mkt spread=Model spread
#Since this is difficult in Python, we can set lambda to the conditional default probability to match
#Calculate conditional default probabilities (continuous time)

#def condit (Q_calc):
    #z = np.zeros(range(len(Q_calc)))
    #for i in range (len(Q_calc)):
        #if (i == 0):
            #z[i] = 1
        #else:
            #z[i] = 1-(Q_calc[i]/Q_calc[i-1])
    #return z
